...
/**
 * Fullscreen modal sample preview. Loads `docx-preview` + `jszip` from the
 * jsDelivr CDN, fetches the server-truncated 5-page DOCX from
 * `/api/marketplace/preview-file/<slug>`, and renders it preserving Word
 * formatting (fonts, tables, images, page layout).
 *
 * Controlled component — parent owns the `open` state and the trigger button.
 */
export function AssetSamplePreviewModal({
  slug,
  title,
  open,
  onClose,
